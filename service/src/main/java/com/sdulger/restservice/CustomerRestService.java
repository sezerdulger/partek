...
import com.sdulger.interceptor.CustomerInterceptor;
...
public class CustomerRestService {
  ...
  @Autowired
  public CustomerInterceptor interceptor;
  @Autowired
  public GraphDatabase db;
  
  @RequestMapping(
      value = "customer.json/page/{page}/limit/{limit}/query/{query}",
  ...
  public ResponseMessage index(@PathVariable("page") int page, @PathVariable("limit") int limit, @PathVariable("query") String query) {
    ...
    Page<Customer> models = null;
    Iterator<Customer> customerList = null;
    Long count = null;
    try {
      PageRequest pageRequest = new PageRequest(page, limit);
      if (query != null && "none".equals(query)) {
        models = repository.findAll(pageRequest);
        customerList = models.iterator();
        ...
        for (String q : queries) {
          ...
          String value = qs[1];
          map.put(field, value);
          Field f = Customer.class.getDeclaredField(field);
        ...
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:Customer) WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        else {
          execute = "MATCH (n:Customer)," + executeMatch.toString() + " WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        System.out.println(execute);
        Result<Customer> customerResult = repository.query(execute, map);
        customerList = customerResult.iterator();
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:Customer) WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        else {
          execute = "MATCH (n:Customer)," + executeMatch.toString() + " WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        System.out.println(execute);
      ...
      while (customerList.hasNext()) {
        customerVector.addElement(customerList.next());
      }
      tx.success();
      rm.restList = customerVector;
      rm.count = count;
    }
    catch(Exception ex) {
      ex.printStackTrace();
  ...
  public ResponseMessage findAll() {
    ...
    try {
      ...
      Iterator<Customer> ss = s.iterator();
      while(ss.hasNext()) {
  ...
  public ResponseMessage delete(@RequestBody Customer model) {
    Transaction tx =db.beginTx();
    ResponseMessage rm = new ResponseMessage();
    try {
