...
import com.sdulger.interceptor.UserInterceptor;
...
public class UserRestService {
  ...
  @RequestMapping(
      value = "user.json/page/{page}/limit/{limit}/query/{query}",
    ...
    try {
      ...
      if (query != null && "none".equals(query)) {
        ...
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:User) WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        else {
          execute = "MATCH (n:User)," + executeMatch.toString() + " WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        System.out.println(execute);
        Result<User> userResult = repository.query(execute, map);
        userList = userResult.iterator();
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:User) WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        else {
          execute = "MATCH (n:User)," + executeMatch.toString() + " WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        System.out.println(execute);
      ...
      while (userList.hasNext()) {
        userVector.addElement(userList.next());
      }
      tx.success();
      rm.restList = userVector;
      rm.count = count;
    }
    catch(Exception ex) {
      ex.printStackTrace();
  ...
  public ResponseMessage delete(@RequestBody User model) {
    Transaction tx =db.beginTx();
    ResponseMessage rm = new ResponseMessage();
    try {
