...
import com.sdulger.interceptor.EmployeeInterceptor;
...
public class EmployeeRestService {
  ...
  @Autowired
  public EmployeeInterceptor interceptor;

  @Autowired
  public GraphDatabase db;
  
  @RequestMapping(
      value = "employee.json/page/{page}/limit/{limit}/query/{query}",
  ...
  public ResponseMessage index(@PathVariable("page") int page, @PathVariable("limit") int limit, @PathVariable("query") String query) {
    ...
    Page<Employee> models = null;
    Iterator<Employee> employeeList = null;
    Long count = null;
    try {
      PageRequest pageRequest = new PageRequest(page, limit);
      if (query != null && "none".equals(query)) {
        models = repository.findAll(pageRequest);
        employeeList = models.iterator();
        ...
        for (String q : queries) {
          ...
          String value = qs[1];
          map.put(field, value);
          Field f = Employee.class.getDeclaredField(field);
        ...
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:Employee) WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        else {
          execute = "MATCH (n:Employee)," + executeMatch.toString() + " WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        System.out.println(execute);
        Result<Employee> employeeResult = repository.query(execute, map);
        employeeList = employeeResult.iterator();
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:Employee) WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        else {
          execute = "MATCH (n:Employee)," + executeMatch.toString() + " WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        System.out.println(execute);
      ...
      while (employeeList.hasNext()) {
        employeeVector.addElement(employeeList.next());
      }
      tx.success();
      rm.restList = employeeVector;
      rm.count = count;
    }
    catch(Exception ex) {
      ex.printStackTrace();
  ...
  public ResponseMessage findAll() {
    ...
    try {
      ...
      Iterator<Employee> ss = s.iterator();
      while(ss.hasNext()) {
  ...
  public ResponseMessage delete(@RequestBody Employee model) {
    Transaction tx =db.beginTx();
    ResponseMessage rm = new ResponseMessage();
    try {
