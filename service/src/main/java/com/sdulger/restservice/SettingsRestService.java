...
public class SettingsRestService {
  ...
  @Autowired
  public SettingsInterceptor interceptor;

  @RequestMapping(
      value = "settings.json/page/{page}/limit/{limit}/query/{query}",
  ...
  public ResponseMessage index(@PathVariable("page") int page, @PathVariable("limit") int limit, @PathVariable("query") String query) {
    ...
    Page<Settings> models = null;
    Iterator<Settings> settingsList = null;
    Long count = null;
    try {
      PageRequest pageRequest = new PageRequest(page, limit);
      if (query != null && "none".equals(query)) {
        models = repository.findAll(pageRequest);
        settingsList = models.iterator();
        ...
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:Settings) WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        else {
          execute = "MATCH (n:Settings)," + executeMatch.toString() + " WHERE(" + executeWhere.toString() + ") RETURN n  SKIP {skip} LIMIT {limit}";
        }
        System.out.println(execute);
        Result<Settings> settingsResult = repository.query(execute, map);
        settingsList = settingsResult.iterator();
        if(executeMatch.toString().equals("")) {
          execute = "MATCH (n:Settings) WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        else {
          execute = "MATCH (n:Settings)," + executeMatch.toString() + " WHERE (" + executeWhere.toString() + ") RETURN count(*)";
        }
        System.out.println(execute);
        count = template.query(execute, map).to(Long.class).single();
      }
      Vector<Settings> settingsVector = new Vector<>();
      while (settingsList.hasNext()) {
        settingsVector.addElement(settingsList.next());
      }
      tx.success();
      rm.restList = settingsVector;
      rm.count = count;
    }
    catch(Exception ex) {
      ex.printStackTrace();
  ...
  @RequestMapping(
      value = "settings.json/{id}",
      method = RequestMethod.GET
  )
  public Settings get(@PathVariable("id") Long id) {
    Settings model = null;
    Transaction tx =db.beginTx();
    try {
  ...
  public Settings save(@RequestBody Settings model) {
    Transaction tx =db.beginTx();
    try {
  ...
  public ResponseMessage findAll() {
    ...
    try {
      ...
      Iterator<Settings> ss = s.iterator();
      while(ss.hasNext()) {
  ...
  public ResponseMessage delete(@RequestBody Settings model) {
    Transaction tx =db.beginTx();
    ResponseMessage rm = new ResponseMessage();
    try {
